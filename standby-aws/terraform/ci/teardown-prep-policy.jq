...
#   aws_instance.standby        disable_api_termination true  -> false
...
| ([.resource_changes[]? | select(.address == "aws_instance.standby")]) as $nodes
| ([.resource_changes[]? | select(.address == "aws_s3_bucket.staging[0]")]) as $staging
| ($nodes | length) == 1
  and ($nodes[0] | exact_protected_state("aws_instance.standby"; "aws_instance"; "disable_api_termination"; true; false))
  and ($changes | length) >= 1
  and ($changes | all(
    exact_flip("aws_instance.standby"; "aws_instance"; "disable_api_termination"; true; false)
